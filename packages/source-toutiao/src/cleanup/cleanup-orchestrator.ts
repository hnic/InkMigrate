 ...
 *
 * 单一事实来源警示：本联合类型与 @inkmigrate/core storage/schema.ts 的
 * CLEANUP_ITEM_PRECHECK_STATUSES（cleanup_items.precheck_status 的 CHECK 约束）
 * 必须逐字一致——改这里必须同步改那里（反之亦然），否则 itemsRepo.upsert 会在
 * 运行时被 CHECK 拒绝，且报错远离类型定义（schema 侧 CONTENT_KINDS 曾因同样
 * 漂移漏掉 'external-link' 造成真实 bug）。
 *
 * 消费面：cleanup_items.precheck_status / cleanup_items.pre_action_state /
 * cleanup_action_attempts.pre_action_state（仓储侧这些列按普通字符串存储）。
 */
...
export async function runCleanupUnfavorite(
  ...
  // 先过滤已成功项再切 limit：已处理项不应消耗本轮配额，否则首批按
  // source_position 排序全是已完成项时本轮会处理 0 条，且随 alreadyDone
  // 增长每轮推进的新条目越来越少
  const rows = allRows
  ...
  // excluded 与 allRows 求交：alreadyDone 只按 sourceInstanceId 圈定，可能含
  // 本轮候选查询范围外的条目（如遗留的 video/external-link、状态已变的项），
  // 直接取全集会让 plan 计数无法与候选快照对账（candidateCount + excludedCount ≠ 快照行数）
  const excludedRows = allRows.filter((r) => alreadyDone.has(r.id));
  const excluded = excludedRows.map((r) => ({
    sourceItemId: r.id,
    reason: 'already_unfavorited',
  }));
  // config_hash 只算一次、plan 报告与 DB 行共用同一值：两处各写一份表达式，
  // 任一侧后续被单独改动就会静默分叉，破坏配置变更审计比对
  const configHash = sourceContentHash(
  ...
  // jobId 加 crypto 随机后缀：同毫秒并发的两个清理任务（并行工作区/测试/时钟
  // 粒度）会撞 cleanup_jobs 主键并共用同一报告目录
  const jobId = `cleanup-${Date.now()}-${randomUUID().slice(0, 8)}`;
  ...
  // 循环内异常也必须收尾 job：persistItem 落库/事务失败若直接上抛，cleanup_jobs 会
  // 永远停在 status='running'（无最终计数、无 finishedAt），断点续跑与 GUI 看板
  // 全部失效。先落当前计数并标 interrupted（CLEANUP_JOB_STATUS 无 'failed'，
  // 中断态是语义最接近的合法终态）再原样重抛。
  try {
    for (let i = 0; i < rows.length; i++) {
      ...
      const persistItem = () => {
        ...
        // 事务提交成功后再计 processedCount（§缺陷2 对账准确性）：事务内先递增
        // 会在回滚时让计数与实际落库条目漂移
        processedCount++;
      };
      ...
      // §5/§14.12 受控中断契约同样覆盖重试轮：重试中检测到登录墙/风控挑战时与首轮
      // 同路径受控中断（此前 hardFailed=false 被误判为成功——条目按 login_required
      // 落库却不计任何桶、不设 pauseReason，任务在登出状态下继续处理后续条目）。
      ...
      if (first.hardFailed && !opts.isCancelled?.()) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
          ...
          // 重试轮的受控中断：与首轮检测同路径（计数/原因/日志一致）
          if (retry.detectedState === 'login_required' || retry.detectedState === 'challenge_required') {
            loginPauseCount++;
            retryPaused = true;
            pauseReason = retry.detectedState;
            opts.onLog?.({
              level: 'warn',
              message:
                retry.detectedState === 'login_required'
                  ? '检测到登录失效，受控中断任务（请重新登录后重跑）'
                  : '检测到风控验证挑战，受控中断任务（稍后重跑）',
            });
            break;
        ...
        if (opts.isCancelled?.()) {
          // 取消可能发生在最后一条的重试等待中：外层 for 不会再回到顶部检查，
          // 必须在此置 terminated，否则任务被误标 completed
          terminated = true;
        }
        // 重试循环结束后仍处于失败态 → 放弃该条，继续下一条（不置 terminated，外层 for 继续推进）
        if (!retryPaused && !opts.isCancelled?.() && stillFailed) {
          opts.onLog?.({
            level: 'error',
      ...
      // 重试轮受控中断：当前条已按 login_required/challenge_required 落库，
      // 此处终止任务（否则外层 for 会继续处理后续条目）
      if (retryPaused) {
        terminated = true;
        break;
      }
    ...
    new CleanupJobs(db).updateStatus(jobId, { status: 'interrupted', finishedAt: t, updatedAt: t });
    throw err;
  }
  // 5. 收尾
  ...
  // 计数与终态放同一事务：分两次写若中途崩溃，会留下"计数已更新但 status 仍
  // running"的中间态，GUI 看板会把它当进行中任务
  db.transaction(() => {
    new CleanupJobs(db).updateCounts(jobId, {
      processedCount,
      successCount,
