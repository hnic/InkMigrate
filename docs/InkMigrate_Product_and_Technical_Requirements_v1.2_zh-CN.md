...
以下映射表仅适用于来源适配器已经成功返回 `SourceItem`、并进入目标写入/验证流程的情况。若 `extract()` 在产出 `SourceItem` 前抛出领域错误，则由错误模型中的 `retryable`、`category` 和错误码直接映射为 `retryable_failed`、`permanent_failed`、`unsupported` 或 `blocked`，不进入本表。
...
| `replace` | 在原路径原子更新并重新验证。 | 仅在配置显式选择该策略时允许覆盖，并记录审计信息。 |
| `write-new` | 在原路径原子更新并重新验证。 | 保留原文件，写入 `.imported-new.md` 或稳定的新 Artifact 路径，并分别记录。 |
...
`candidate_quality`、`candidate_degradations_json` 和 `candidate_source_content_hash` 用于保存本次尝试尚未提交的标准化结果；成功完成目标验证后，核心在事务中把它们提交到 `source_items`。不得记录 Cookie、Authorization、验证码、手机号或完整请求头。
...
4. 目标未被用户修改，且策略为 `preserve`、`replace` 或 `write-new` 时，允许原子更新目标内容；目标验证通过后，才将最终状态改为 `verified`、提交新的 `quality: full`、清空旧降级原因并更新三类哈希。
5. 目标已被用户修改时：`preserve` 产生 `conflict`；`replace` 按显式策略覆盖并审计；`write-new` 保留原文件并创建新的已验证 Artifact。
...
16. Evernote 契约 Fixture 可被核心注册但 v1.0 不要求完整迁移。
