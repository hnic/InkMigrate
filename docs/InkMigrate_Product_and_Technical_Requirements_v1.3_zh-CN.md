...
| `replace` | 在原路径原子更新并重新验证。 | 仅在配置显式选择该策略时允许覆盖；必须按下述字段写入强制覆盖审计。 |
...
当 `replace` 覆盖一个已被用户或外部程序修改的文件时，审计信息必须写入 `migration_attempts`，至少包括：

```text
action_code = forced_overwrite
target_artifact_id
overwrite_policy = replace
expected_written_file_hash
observed_prewrite_file_hash
result_written_file_hash
audit_metadata_json
```

核心必须在覆盖前创建尝试记录并保存期望哈希与实际观察哈希；目标文件原子写入和验证成功后再提交结果哈希及 `success = 1`。写入或验证失败时保留 `success = 0` 的尝试记录，不得丢失覆盖意图和覆盖前证据。`audit_metadata_json` 至少记录触发命令、冲突原因码和相对路径，不得包含 Cookie、Token 或正文内容。
...
该 Artifact 还必须通过当前磁盘复核。索引、报告或 `source_item_id IS NULL` 的 Artifact 不能证明条目属于该 Migration Job。
...
- `action_code` 的核心保留值至少包括 `stage_attempt`、`source_update`、`quality_upgrade`、`forced_overwrite`、`write_new_variant` 和 `metadata_update`；适配器扩展遵循 §16.6 相同的命名空间规则。
...
1. 将本次结果标记为 `quality_upgrade_candidate`；不能仅修改状态为 `verified`。
...
5. 目标已被用户修改时：`preserve` 产生 `conflict`；`replace` 按显式策略覆盖，并在 `migration_attempts` 以 `action_code = 'forced_overwrite'` 记录 `target_artifact_id`、覆盖策略、期望哈希、覆盖前观察哈希和验证后的结果哈希；`write-new` 保留原文件并创建新的已验证 `note_variant` Artifact。
6. `metadata-only` 不允许完成涉及正文或附件完整性的质量升级；条目保持原 `degraded` 结果，并报告 `quality_upgrade_deferred`。
...
- Job 级配置、认证、限流、安全验证或内部系统错误可以不提供 `itemDisposition`，按 Job 生命周期处理。
...
17. `replace` 覆盖用户修改文件时，成功与失败路径都留下包含覆盖前后哈希的 `forced_overwrite` 审计记录。
...
20. Evernote 契约 Fixture 可被核心注册但 v1.0 不要求完整迁移。
...
>
> 条目级不可重试错误必须通过 `itemDisposition` 显式声明 `permanent_failed`、`unsupported` 或 `blocked`；核心不得根据错误消息猜测。`migration_attempts` 必须区分 Job/条目作用域，并为 `source_item_id IS NULL` 与非空场景分别建立部分唯一索引。`artifact_kind` 使用受控值，canonical 内容笔记统一为 `note`，冲突变体统一为 `note_variant`。
>
